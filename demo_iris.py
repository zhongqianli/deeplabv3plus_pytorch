...
def foreach_files(root_dir):
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith(".jpg") or file.endswith(".bmp") or file.endswith(".png") or file.endswith(".JPEG"):
...
def main():
    # model_file = "run/iris_seg/deeplab-mobilenet/model_best.pth.tar"
    # model = DeepLab(backbone='mobilenet', num_classes=2, output_stride=8)

    model_file = "run/iris_seg/deeplab-xception/model_best.pth.tar"
    model = DeepLab(backbone='xception', num_classes=2, output_stride=8)
